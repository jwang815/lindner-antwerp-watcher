...
def check_trip(page):
    try:
        ...
        # IMPORTANT ordering: when this hotel is sold out, Trip.com shows a
        # "not currently accepting bookings" / "View Other Hotels" banner AND a
        # nearby-hotels upsell that contains a stray Reserve button + a "Total
        # price" string. So the sold-out banner is authoritative and must be
        # checked BEFORE any positive room signal, or the upsell false-fires.
        if "not currently accepting bookings" in low:
            log("[trip] SOLD OUT (not-accepting-bookings banner)")
        ...
        # A genuinely bookable page lists many room-rate rows (each rate plan is
        # its own Reserve button -- typically dozens). Require >=2 + a price so a
        # single stray upsell button can never trigger an alert.
